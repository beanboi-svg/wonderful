(async function renderTopWord() {
    const svg = document.getElementById('dynamic-word-top');
    const pathEl = document.getElementById('dynamic-word-top-path');
    if (!svg || !pathEl) return;

    const n = Math.floor(Math.random() * 4) + 1;
    const file = `assets/t${n}.txt`;
    ...
    try {
        ...
        console.error('Could not load top word svg:', err);
    }
})();